...
export function generatePrompt(activity, question, transcript) {
  const message = [
    ...
    new HumanMessage(`You are an examiner evaluating a candidate's speaking test. The candidate spoke on the topic "${question}" for the activity "${activity}". Your task is to assess the candidate's performance and provide a detailed report.
Evaluation Criteria:
...
Content Adequacy: Determine whether the candidate spoke enough on the topic. If the response was too short or lacked depth, penalize accordingly.
Additional Notes:
...
If the speech lacks sufficient content, reflect this in all relevant areas (fluency, coherence, etc.) and note it explicitly in the evaluation.
Transcript: The candidate’s speech transcript is provided below:
${transcript}

Format: Return the evaluation in JSON format using the following structure:
{
    "report": {
        "candidate_performance": {
            ...
            "content_adequacy": { "score": null, "evaluation": "" }
        },
...
If the candidate's response is too short, penalize fluency, coherence, and content adequacy, and explicitly mention the issue.
Use the provided JSON template without modification.`),
  ];
